...
> #6、#8 已固化为设计规则 / 已完成（见上）。**#7、#9、#10、#11 已在功能迭代阶段完成（2026-06-25）**，详见各项。
...
### 7. 减少 FBX 重复加载
- ✅ **已完成（2026-06-25）**：分析阶段加载的场景保留（pristine）为导出参考，导出处不再重复加载；单个输入文件解析 3→2 次。导出/重映射/校验算法不变，样例回归 0 mismatch。

### 8. 参数化"8×8 块大小"
- ✅ **已完成**：魔法数 8 已抽为全局常量 `core::kTextureBlockSize`（见上方「已确立的设计规则」），`uv/UVAnalyzer` 的量化与区域矩形已改为引用该常量。

### 9. 跨平台（抽接口方案）
- ✅ **已完成（2026-06-25）**：GDI+ 图片读写移入 [atlas/ImageBackend_Windows.cpp](../atlas/ImageBackend_Windows.cpp)（后端接口 `DecodeImageOS`/`EncodePngOS`），`conio` 暂停移入 [core/Console.cpp](../core/Console.cpp)；`AtlasBuilder`/`main` 等不再包含 Windows 头。零新依赖、行为不变；将来真正移植只需新增一个后端 TU（如 stb）。
- **未做**：非 Windows 的 CMake/CI 与 FBX SDK 跨平台（FBX SDK 本身平台相关），等真有 Linux/Mac 需求再启动。

### 10. CLI 体验
- ✅ **已完成（2026-06-25）**：`-h` 改为 help（破坏性变更，已在提交说明标注）；宽高用 `--width` / `--height`，新增 `--wid` / `--hei` 别名（`-w` 仍兼容）；帮助页增强为说明 + 对齐选项 + 注意事项 + 示例。新增对应解析测试。

### 11. 装箱算法升级
- ✅ **已完成（2026-06-25）**：shelf 行式装箱换为 **skyline 底-左**装箱，降低纵向浪费、提升利用率，并保持不重叠 / 不越界；新增密集装箱回归测试。小样例仍为 256×256（两种算法都在同一 2 的幂预算内），利用率收益在更密的瓦片集上体现。
